        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_p:
                    board.predict(screen)

        screen.fill((255, 255, 255, 0))
        clock.tick(fps)
        if board.predicted:
            prediction = np.argmax(board.inputs)
            board.display_text(str(prediction), 10, screen, 50, x=(board.width - 1) * board.cell_size + board.left)
            a = board.inputs.copy()
            a[prediction] = 0.0
            #print(board.inputs)
            board.display_text(str(np.argmax(a)), 10, screen, 50, x=5)

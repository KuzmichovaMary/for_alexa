...
class Board:
    ...
    def render(self, screen):
        ...
        # pygame.draw.rect(screen, (0, 0, 0), [self.left + self.cell_size * 4, self.top + self.cell_size * 4,
        #                                      self.cell_size * (self.width - 8),
        #                                      self.cell_size * (self.height - 8)], 1)
    ...
    def on_click(self, cell_coords):
        if cell_coords:
            i, j = cell_coords
            brush_line(self.prev_x, self.prev_y, j, i, self.board, 1)
    ...
    def display_text(self, text, y, screen, font_size=50, pos="align left", x=10, width=200):
    ...
    def predict(self, screen):
        with open("weights (1).pkl", "rb") as file:
            W = pickle.load(file)
        b = [[[] for _ in range(self.width)] for _ in range(self.height)]
        for i in range(self.height):
            for j in range(self.width):
        ...
        img = np.array(b, dtype=np.uint8)
        img = Image.fromarray(img).convert("L")
        inputs_list = list(map(str, list(np.asarray(img).flatten())))
        inputs = np.array(inputs_list, dtype=np.int).reshape(len(inputs_list), 1)
        for weights_matrix in W[:1]:
            inputs = self.activation1(weights_matrix @ inputs)
        self.predicted = True
        self.inputs = self.activation2(W[1] @ inputs)
        return list(map(lambda a: '{0:.10f}'.format(a), self.inputs.flatten()))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_p:
                    w = board.predict(screen)

        screen.fill((255, 255, 255, 0))
        clock.tick(fps)
        if board.predicted:
            prediction = np.argmax(board.inputs)
            board.display_text(d[str(prediction)].capitalize(), 10, screen, 30, width=width)
            board.display_text("Probability: " + w[prediction], 40, screen, 30, width=width)
